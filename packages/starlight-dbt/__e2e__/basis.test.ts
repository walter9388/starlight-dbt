...
test.describe('Sidebar Functionality', () => {
	...
	test.describe('Sidebar Ordering and Metadata', () => {
		test('should maintain correct ordering of Starlight vs dbt-plugin items', async ({
			page,
			getProdServer,
		}) => {
			const starlight = await getProdServer();
			...
			// Target top-level items in the sidebar
			const topLevelItems = page.locator('.top-level > li');

			// First "Examples" (Standard Starlight)
			await expect(topLevelItems.nth(0)).toContainText('Examples');

			// "Default dbt Project" (dbt plugin slug: 'default')
			await expect(topLevelItems.nth(1)).toContainText('Default dbt Project');

			// Second "Examples" (dbt plugin autogenerated)
			await expect(topLevelItems.nth(2)).toContainText('Examples');
		});

		test('should pass through attributes and badges for plugin-defined sidebar items', async ({
			page,
			getProdServer,
		}) => {
			const starlight = await getProdServer();
			...
			// The third top-level item (the second "Examples" group)
			const dbtExamplesGroup = page.locator('.top-level > li').nth(2);
			const exampleLink = dbtExamplesGroup.locator('details > ul > li > a');

			// Verify "attrs" (inline styles)
			await expect(exampleLink).toHaveAttribute('style', /background-color: orange/);
			await expect(exampleLink).toHaveAttribute('style', /font-style: italic/);

			// Verify "badge" (Starlight renders badges inside a span with the .sl-badge class)
			const badge = dbtExamplesGroup.locator('.sl-badge');
			await expect(badge).toBeVisible();
			await expect(badge).toHaveText('new');
		});

		test('should correctly link autogenerated items within the plugin group', async ({
			page,
			getProdServer,
		}) => {
			const starlight = await getProdServer();
			...
			const pluginExamplesGroup = page.locator('.top-level > li').nth(2);
			const exampleLink = pluginExamplesGroup.locator('details > ul > li > a');

			await expect(exampleLink).toBeVisible();
			await expect(exampleLink).toHaveAttribute('href', '/examples/example1/');
		});
	});
});
